document.addEventListener('DOMContentLoaded', () => {
  ...
  // Show promotion banner for weekend bookings
  ...
  dateInput.addEventListener('change', () => {
    ...
    if (!isNaN(selectedDate)) {
      ...
      if (day === 0 || day === 6) {
        if (promotionBanner) promotionBanner.style.display = 'block';
      } else {
        if (promotionBanner) promotionBanner.style.display = 'none';
      }
    } else {
      if (promotionBanner) promotionBanner.style.display = 'none';
    }
  ...
  // Function to update price display with discount if applicable
  function updatePriceDisplay() {
    ...
    serviceHelp.textContent = `Price: RM ${price.toFixed(2)}`;
  }
